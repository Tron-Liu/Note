...
bool HttpConn::write() {
  ...
  while (1) {
    ...
    if (bytes_to_send <= bytes_have_send) {
      // 发送 HTTP 响应成功，根据 HTTP 请求中的 Connection
      // 字段决定是否立即关闭连接
      unmap();
      if (m_linger) {
        init();
...
HttpConn::HTTP_CODE HttpConn::parse_request_line(char* text) {
  ...
  *m_url++ = '\0';
  ...
  if (strcasecmp(m_version, HTTP_VERSION_1_1) != 0) return BAD_REQUEST;
...
// 此处没有真正解析 HTTP 请求的消息体，只是判断它是否被完整地读入了
