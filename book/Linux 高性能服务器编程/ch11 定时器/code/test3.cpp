...
void sig_handler(int sig) {
  ...
  int msg = sig;
  send(pipefd[WRITE], (char*)&msg, 1, 0);
  errno = save_errno;
}
...
// 定时器回调函数，删除非活跃连接 socket 上的注册事件，并关闭之
...
int main(int argc, char* argv[]) {
  ...
  assert(ret != -1);
  setnonblocking(pipefd[WRITE]);
  addfd(epollfd, pipefd[READ]);
    ...
    for (int i = 0; i < number; i++) {
      ...
      // 处理客户连接上接收到的数据
        ...
        if (ret < 0) {
          ...
          // 如果对方已经关闭了连接，则我们也关闭连接，并移除对应的定时器
          cb_func(&users[sockfd]);
